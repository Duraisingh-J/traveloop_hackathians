...
import { GripVertical, MapPin, Plus, Trash2, CheckCircle2, Search } from 'lucide-react'
...
export default function ItineraryBuilderPage() {
  ...
  const handleAddStop = async (e: React.FormEvent) => {
    ...
    try {
      // Simulate Geocoding or fetching from our Supabase DB
      ...
      if (data && data.length > 0) {
        ...
        // Fallback dummy coords if city not found in DB
  ...
  const handleSave = async () => {
    ...
    try {
      // 1. Create Trip
      // 2. Insert Stops
      // (Simplified logic, assuming trip is created on previous page or here)
          ...
          <CardContent>
            <form onSubmit={handleAddStop} className="flex gap-2">
              <Input 
                ...
                disabled={searching}
              />
              <Button type="submit" disabled={searching || !newCity.trim()} className="whitespace-nowrap">
                {searching ? <Search className="h-4 w-4 mr-2 animate-spin" /> : <Plus className="h-4 w-4 mr-2" />}
                Add
              </Button>
            </form>
          </CardContent>
        ...
        <Card className="shrink-0">
          <CardContent className="p-6">
            ...
            <Button 
              ...
              onClick={handleSave}
            >
              {saving ? <Search className="h-4 w-4 mr-2 animate-spin" /> : <CheckCircle2 className="mr-2 h-4 w-4" />} 
              {saving ? 'Saving to Database...' : 'Save Itinerary'}
            </Button>
          </CardContent>
        </Card>
      </div>
